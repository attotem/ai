...
function All_categories() {
    ...
    useEffect(() => {
        fetch("https://dashboard-dmitrykarpov.pythonanywhere.com/get_all_categories/", {
            ...
            .then(data => {
                // Предположим, что data уже является объектом JavaScript.
                // Если data - это строка, которую нужно распарсить, используйте JSON.parse(data)
                setCustomersData(JSON.parse(data)); // Установка данных напрямую, без JSON.parse, если ответ уже в формате JSON
            })
            .catch(error => {
